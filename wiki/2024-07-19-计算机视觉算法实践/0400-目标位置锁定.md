---
vinciId: dc5551e0-8232-480e-9691-7c7f921cb69d
...
按现行系统分层，软件算法组负责从图像或其他传感器得到目标位置、姿态、角度误差或轨迹，并生成定义清楚的期望速度或位置；嵌入式组负责电机速度环、位置环和执行机构的实时闭环。PID运行在哪一端取决于控制对象和实时性要求：上位机可以实现目标跟踪或非实时运动控制策略，MCU负责高频电机闭环、安全限幅和失联保护，避免两端对同一对象重复闭环。
